  ...
  <div class="container-fluid mt-5">
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h4>Tipo Sensor</h4>
        <div>
          <input type="text" class="form-control d-inline-block w-auto" placeholder="Buscar por nome ou ID..." v-model="searchQuery">
        ...
        <table class="table table-bordered">
          ...
          <tbody v-if="paginatedItems.length > 0">
            <tr v-for="(item, index) in paginatedItems" :key="index">
              ...
              <td>{{ item.Nome }}</td>
              <td class="text-right">
                <router-link :to="{ path: '/tiposensor/' + item.ID + '/edit' }" class="btn btn-success">
                  <i class="fa fa-pencil" aria-hidden="true"></i> Editar
                </router-link>
                <button type="button" @click="ShowConfirmDelete(item.ID)" class="btn btn-danger">
...
module.exports = {
  ...
  computed: {
    ...
    filteredItems() {
      ...
      return this.items.filter(item => {
        return item.Nome.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
  ...
  methods: {
    ...
    deleteItem(ItemID) {
      axios.delete(`/rs2lab/deletetiposensor/${ItemID}`)
        .then(() => {
